...
class StateMachineOperatorTest {
    ...
    void setupOperator() {
        ...
        operator = new StateMachineOperator(state);
    }
    ...
    @Test
    void testExecuteOperation_firstRequest_executesSuccessfully() {
        MessageServiceOuterClass.ClientRequest request = createTransferRequest("client1", 100L, "A", "B", 5.0);

        MessageServiceOuterClass.ClientReply reply = operator.executeOperation(request, 1L);
    ...
    @Test
    void testExecuteOperation_sameSequenceNumber_doesNotExecuteTwice() {
        ...
        // Execute first time
        MessageServiceOuterClass.ClientReply reply1 = operator.executeOperation(request1, 1L);
        ...
        MessageServiceOuterClass.ClientReply reply2 = operator.executeOperation(request2, 1L);
        assertNull(reply2, "Second execution with same seqNum should return null");
    }

    @Test
    void testExecuteOperation_lowerSequenceNumber_doesNotExecute() {
        MessageServiceOuterClass.ClientRequest request1 = createTransferRequest("client1", 100L, "A", "B", 5.0);
        MessageServiceOuterClass.ClientRequest request2 = createTransferRequest("client2", 200L, "A", "B", 10.0);

        // Execute seqNum 3
        operator.executeOperation(request1, 3L);
        operator.executeOperation(request1, 1L);
        operator.executeOperation(request1, 2L);

        // Try to execute seqNum 2 (lower than last executed)
        MessageServiceOuterClass.ClientReply reply = operator.executeOperation(request2, 2L);
        assertNull(reply, "Operation with lower seqNum should return null");
    }

    @Test
    void testExecuteOperation_outOfOrder_pendingExecution() {
        MessageServiceOuterClass.ClientRequest request1 = createTransferRequest("client1", 100L, "A", "B", 5.0);

        // Try to execute seqNum 3 without executing 1 and 2 first
        MessageServiceOuterClass.ClientReply reply = operator.executeOperation(request1, 3L);

        assertNull(reply, "Out-of-order operation should return null (pending)");
    }

    @Test
    void testExecuteOperation_sequentialExecution_allExecuted() {
        MessageServiceOuterClass.ClientRequest request1 = createTransferRequest("client1", 100L, "A", "B", 5.0);
        ...
        MessageServiceOuterClass.ClientReply reply2 = operator.executeOperation(request2, 2L);
        assertNotNull(reply2, "Second operation should execute");

        MessageServiceOuterClass.ClientReply reply3 = operator.executeOperation(request3, 3L);
        assertNotNull(reply3, "Third operation should execute");
    }

    @Test
    void testExecuteOperation_outOfOrderThenFillGap_executesAll() {
        MessageServiceOuterClass.ClientRequest request1 = createTransferRequest("client1", 100L, "A", "B", 5.0);
        MessageServiceOuterClass.ClientRequest request2 = createTransferRequest("client2", 200L, "B", "C", 3.0);
        MessageServiceOuterClass.ClientRequest request3 = createTransferRequest("client3", 300L, "C", "A", 2.0);
        ...
        MessageServiceOuterClass.ClientReply reply3Again = operator.executeOperation(request3, 3L);
        assertNull(reply3Again, "Operation already executed should return null");
    }

    @Test
    void testExecuteOperation_multipleOutOfOrderThenFillGaps_executesAll() {
        ...
        assertNull(operator.executeOperation(request3, 3L), "SeqNum 3 should be pending");
        assertNull(operator.executeOperation(request5, 5L), "SeqNum 5 should be pending");

        assertNotNull(operator.executeOperation(request1, 1L), "SeqNum 1 should execute");

        assertNull(operator.executeOperation(request4, 4L), "SeqNum 4 should be pending");

        // Executing seqNum 2 should trigger execution of 3, 4, and 5
        assertNotNull(operator.executeOperation(request2, 2L), "SeqNum 2 should execute");

        // All should now be executed - trying to execute any should return null
        assertNull(operator.executeOperation(request3, 3L), "SeqNum 3 should already be executed");
        assertNull(operator.executeOperation(request4, 4L), "SeqNum 4 should already be executed");
        assertNull(operator.executeOperation(request5, 5L), "SeqNum 5 should already be executed");
    }

    @Test
    void testExecuteOperation_duplicateAttemptsDifferentRequests_onlyFirstExecutes() {
        MessageServiceOuterClass.ClientRequest request1 = createTransferRequest("client1", 100L, "A", "B", 5.0);
    ...
    @Test
    void testMarkExecutedUpTo_advancesExecution() {
        MessageServiceOuterClass.ClientRequest request1 = createTransferRequest("client1", 100L, "A", "B", 5.0);
        MessageServiceOuterClass.ClientRequest request2 = createTransferRequest("client2", 200L, "B", "C", 3.0);
        ...
        assertNull(operator.executeOperation(request2, 2L), "SeqNum 2 should not execute after marking up to 5");

        // But seqNum 6 should execute
        MessageServiceOuterClass.ClientRequest request6 = createTransferRequest("client6", 600L, "A", "B", 1.0);
        assertNotNull(operator.executeOperation(request6, 6L), "SeqNum 6 should execute");
    }

    @Test
    void testReset_clearsExecutionState() {
        MessageServiceOuterClass.ClientRequest request1 = createTransferRequest("client1", 100L, "A", "B", 5.0);
        ...
        MessageServiceOuterClass.ClientReply reply = operator.executeOperation(request1, 1L);
        assertNotNull(reply, "After reset, seqNum 1 should be executable again");
    }

    @Test
    void testExecuteOperation_verifyStateChanges() {
        ...
        MessageServiceOuterClass.ClientReply transferReply = operator.executeOperation(transferRequest, 1L);
        ...
        MessageServiceOuterClass.ClientReply balanceReplyA = operator.executeOperation(balanceRequestA, 2L);
        ...
        MessageServiceOuterClass.ClientReply balanceReplyB = operator.executeOperation(balanceRequestB, 3L);
    ...
    @Test
    void testExecuteOperation_duplicatePreventsDoubleSpending() {
        double initialBalanceA = Config.getClientBalances().get("A");
        double initialBalanceB = Config.getClientBalances().get("B");
        ...
        // Execute once
        MessageServiceOuterClass.ClientReply reply1 = operator.executeOperation(transferRequest, 1L);
        assertNotNull(reply1, "First execution should succeed");

        // Try to execute the same seqNum again (simulating duplicate)
        MessageServiceOuterClass.ClientReply reply2 = operator.executeOperation(transferRequest, 1L);
        ...
        MessageServiceOuterClass.ClientReply balanceReply = operator.executeOperation(balanceRequest, 2L);
    ...
    @Test
    void testSnapshot_capturesCurrentState() {
        ...
        operator.executeOperation(request, 1L);
